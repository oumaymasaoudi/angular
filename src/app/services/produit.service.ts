...
export class ProduitService {
  ...
  supprimerProduit( prod: Produit){
    ...
    //ou Bien
    /* this.produits.forEach((cur, index) => { if(prod.idProduit === cur.idProduit) {
    this.produits.splice(index, 1); }
    }); */
  }
  ...
  trierProduits(){
    this.produits = this.produits.sort((n1,n2) => { if (n1.idProduit! > n2.idProduit!) {
    return 1;
    }
    ...
    {
    // console.log(p);
    this.supprimerProduit(p); this.ajouterProduit(p); this.trierProduits();
    }
